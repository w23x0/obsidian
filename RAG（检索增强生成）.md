<<<<<<< HEAD
iji学习资源
OpenClaw 官方文档
记忆系统详解：/home/wang/.npm-global/lib/node_modules/openclaw/docs/concepts/memory.md（我刚读的这个）
完整文档：https://docs.openclaw.ai
RAG（检索增强生成）学习资料
如果你想了解背后的通用技术：
=======
# RAG（检索增强生成）
>>>>>>> origin/main

## 学习资源

### OpenClaw 官方文档
- 记忆系统详解：`/home/wang/.npm-global/lib/node_modules/openclaw/docs/concepts/memory.md`
- 完整文档：https://docs.openclaw.ai

## RAG（检索增强生成）学习资料

### 原始论文
- **Retrieval-Augmented Generation (RAG) 详解** - 原始论文
  - 论文地址：https://arxiv.org/abs/2005.11401
  - 标题：Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks
  - 作者：Patrick Lewis, Ethan Perez, et al.
  - 发表：NeurIPS 2020

### 文章与教程
- **OpenAI RAG 指南**
  - https://platform.openai.com/docs/guides/embeddings/use-cases
  - https://platform.openai.com/docs/tutorials/web-qa-embeddings

- **LlamaIndex 文档** - 流行的 RAG 框架
  - 官网：https://www.llamaindex.ai/
  - 文档：https://docs.llamaindex.ai/
  - GitHub：https://github.com/run-llama/llama_index

- **LangChain 文档** - 另一个流行的 LLM 应用框架
  - 官网：https://www.langchain.com/
  - 文档：https://python.langchain.com/
  - GitHub：https://github.com/langchain-ai/langchain

- **Chroma 文档** - 向量数据库
  - 官网：https://www.trychroma.com/
  - 文档：https://docs.trychroma.com/
  - GitHub：https://github.com/chroma-core/chroma

- **Pinecone 文档** - 托管向量数据库
  - 官网：https://www.pinecone.io/
  - 文档：https://docs.pinecone.io/

- **Weaviate 文档** - 开源向量搜索引擎
  - 官网：https://weaviate.io/
  - 文档：https://weaviate.io/developers/weaviate
  - GitHub：https://github.com/weaviate/weaviate

- **Qdrant 文档** - 向量数据库
  - 官网：https://qdrant.tech/
  - 文档：https://qdrant.tech/documentation/
  - GitHub：https://github.com/qdrant/qdrant

### 视频
- **YouTube 搜索关键词**
  - "RAG tutorial"
  - "retrieval augmented generation"
  - "RAG from scratch"
  - "LlamaIndex tutorial"
  - "LangChain RAG"

- **Andrej Karpathy 的 State of GPT**（提到 RAG）
  - 视频地址：https://www.youtube.com/watch?v=bZQun8Y4L2A
  - 发表时间：2023年

- **其他推荐视频**
  - LangChain 的 RAG 教程：https://www.youtube.com/@LangChain
  - LlamaIndex 的官方频道：https://www.youtube.com/@LlamaIndex
  - 1littlecoder 的 RAG 教程：https://www.youtube.com/@1littlecoder

### 项目与工具
- **QMD** - OpenClaw 用的本地搜索工具
  - GitHub：https://github.com/tobi/qmd
  - 特点：本地优先，结合 BM25 + 向量 + 重排序

- **LlamaIndex**
  - GitHub：https://github.com/run-llama/llama_index

- **LangChain**
  - GitHub：https://github.com/langchain-ai/langchain

- **Ollama** - 本地运行 LLM
  - 官网：https://ollama.com/
  - GitHub：https://github.com/ollama/ollama

- **node-llama-cpp** - Node.js 的 LLM 运行时
  - GitHub：https://github.com/withcatai/node-llama-cpp

- **sqlite-vec** - SQLite 的向量扩展
  - GitHub：https://github.com/asg017/sqlite-vec

### 嵌入模型
- **OpenAI Embeddings**
  - 文档：https://platform.openai.com/docs/guides/embeddings
  - 模型：text-embedding-3-small, text-embedding-3-large

- **Gemini Embeddings**
  - 文档：https://ai.google.dev/docs/embeddings_guide

- **Voyage AI**
  - 官网：https://www.voyageai.com/

- **Mistral AI Embeddings**
  - 文档：https://docs.mistral.ai/guides/embeddings/

- **本地嵌入模型（HuggingFace）**
  - Sentence-BERT：https://www.sbert.net/
  - all-MiniLM-L6-v2：https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2
  - bge-large-zh-v1.5：https://huggingface.co/BAAI/bge-large-zh-v1.5
